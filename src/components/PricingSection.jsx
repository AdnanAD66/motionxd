...
export default function PricingSection() {
  ...
  return (
    <section className="relative bg-gradient-to-br from-neutral-950 via-neutral-900 to-black text-white py-32 px-6 md:px-24 overflow-hidden rounded-3xl">
      ...
      <motion.div
        className="absolute top-0 left-1/2 -translate-x-1/2 w-[1200px] h-[1200px] rounded-full bg-primary/10 blur-[180px] z-0"
        animate={{
          scale: [1, 1.05, 1],
          opacity: [0.5, 0.7, 0.5],
        }}
      ...
      <div className="max-w-6xl mx-auto text-center relative z-10">
        ...
        <motion.div
          ...
          className="mb-20"
        >
          <h2 className="text-5xl md:text-6xl font-extrabold bg-gradient-to-r from-primary via-blue-400 to-primary bg-clip-text text-transparent mb-4 tracking-tight">
            PRICING
          </h2>
          ...
          {plans.map((plan, i) => (
            <motion.div
              ...
              className={`relative backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl shadow-2xl p-10 flex flex-col items-center transition-all duration-300 ${
                plan.highlight
                  ? "bg-gradient-to-b from-primary/20 to-blue-900/30 border-primary/40 shadow-primary/30"
            ...
            >
              ...
              {plan.highlight && (
                <motion.div
                  className="absolute inset-0 bg-gradient-to-tr from-primary/20 via-transparent to-transparent blur-3xl"
                  animate={{
                    opacity: [0.4, 0.8, 0.4],
                    scale: [1, 1.1, 1],
                  }}
              ...
              <h3 className="text-2xl font-semibold mb-3 z-10">{plan.name}</h3>
              <p className="text-5xl font-bold mb-4 z-10 bg-gradient-to-r from-primary to-blue-400 bg-clip-text text-transparent">
                {plan.price}
              </p>
              ...
              <ul className="text-left text-neutral-300 mb-8 space-y-3 z-10">
                {plan.features.map((f, idx) => (
                  <li key={idx} className="flex items-start gap-2">
                    <span className="text-primary text-lg mt-[2px]">•</span>
                    {f}
                  </li>
                ))}
              </ul>
              ...
              <motion.button
                whileHover={{
                  scale: 1.1,
                  backgroundColor: plan.highlight ? "var(--tw-color-primary)" : "#3B82F6",
                  color: "#fff",
                }}
